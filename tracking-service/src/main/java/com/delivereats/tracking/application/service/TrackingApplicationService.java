...
import com.delivereats.tracking.domain.event.TrackingCreatedEvent;
...
public class TrackingApplicationService implements CreateTrackingUseCase, GetTrackingUseCase {
	...
	public TrackingApplicationService(TrackingRepositoryPort trackingRepository, TrackingPublisher trackingPublisher) {
	...
	public TrackingResponse createTracking(CreateTrackingRequest request) {
		...
		trackingPublisher.publish("tracking.created", new TrackingCreatedEvent(request.orderId(), trackingNumber));

		return new TrackingResponse(trackingNumber, TrackingStatus.CREATED.name(), request.riderName(),
				request.estimatedArrival());
	}
